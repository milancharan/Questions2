...
(1, 'Image1', 'http://www.imagegallery/1'),
(2, 'Image2', 'http://www.imagegallery/2'),
(3, 'Image3', 'http://www.imagegallery/3'),
(4, 'Image4', 'http://www.imagegallery/4'),
(5, 'Image5', 'http://www.imagegallery/5
')
...
CREATE VIEW notImage As
SELECT * from Product p
INNER JOIN ProductDetail pd ON p.productID = pd.ProductID
where p.CreatedDate is NOT NULL AND pd.ImageURL is NOT NULL;

SELECT * from notImage;

SELECT ParentID from Category c 
Inner JOIN Product p ON p.CategoryID = c.CategoryID

SELECT * from Product p 
INNER JOIN ProductDetail pd ON p.ProductID = pd.ProductID
WHERE p.Status = 'inStock' AND  pd.ImageURL is NOT NULL

SELECT p.Name SubCategoryName, c.CategoryName ParentCategoryName from Product p
